...
require(CytoTree)  ##repasar a ver si todos estos serían necesarios aquí
require(LSD)
require(ggplot2)

####funciones usadas para el apartado de Quality assessment, definidas en FlowiQC
...
###### Funciones basadas en cytotree

#lectura archivo FCS con compensación incluida en el archivo
    #funcion que toma un archivo y crea un objeto = matriz de expresión del archivo FCS
FCScompensation_inluded <- function(FCSfile) {
    cyt.data <- CytoTree::runExprsExtract(FCSfile, comp = TRUE, transformMethod = "none", showDesc = FALSE)
    return(cyt.data)
} 

#lectura archivo FCS con compensación proporcionada por el usuario
FCScompensation_provided <- function(FCSfile, compensation_matrix) {
    cyt.data <- CytoTree::runExprsExtract(FCSfile, comp = compensation_matrix, transformMethod = "none", showDesc = FALSE)
    return(cyt.data)
} 
